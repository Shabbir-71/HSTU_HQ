...
<head>
    ...
    <title>HSTU HQ | Admin Upload Database</title>
  ...
  <div class="result-body">
      <?php
          ...
          if(isset($_POST['delete_row'])){
            ...
            $ques_id = $_POST['q_id'];
            $sql1 = "DELETE FROM upload WHERE upload_id = '" . $ques_id . "'";
          ...
          $sql = "SELECT upload_id, student_id, link FROM upload ORDER BY upload_id ASC";
          $result = $conn->query($sql);

          echo"<table class='content-table'>";
            echo"<thead><tr><th>Upload ID</th><th>Student ID</th><th>Download Link</th><th>Delete</th></tr></thead><tbody>";
              if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                  echo "<tr><td>". $row["upload_id"] . "</td><td>" . $row["student_id"] . "</td><td><a target='_blank' href='" . $row["link"]. "'>Download</a></td><td><form action='' method='post'><input type='hidden' name='q_id' value='" . $row["upload_id"] . "'><input type='submit' name='delete_row' value='delete'></form></td></tr>\n";
                }
              }
            echo"</tbody></table>";
