...
<head>
    ...
    <title>HSTU HQ | Admin Database</title>
  ...
  <div class="result-body">
      <?php
          ...
          if(isset($_POST['delete_row'])){
            ...
            $ques_id = $_POST['q_id'];
            $sql1 = "DELETE FROM question WHERE question_id = '" . $ques_id . "'";
          ...
          echo"<table class='content-table'>";
            echo"<thead><tr><th>Question ID</th><th>Year</th><th>Level</th><th>Semester</th><th>Degree</th><th>Department</th><th>Course Code</th><th>Course Title</th><th>Download Link</th><th>Edit</th><th>Delete</th></tr></thead><tbody>";
              if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                  echo "<tr><td>". $row["question_id"] . "</td><td>" . $row["year_var"]. "</td><td>" . $row["level_var"]. "</td><td>" . $row["semester"]. "</td><td>" . $row["degree"]. "</td><td>" . $row["department"]. "</td><td>" . $row["course_code"]. "</td><td>" . $row["course_title"]. "</td><td><a target='_blank' href='" . $row["link"]. "'>Download</a></td><td><form action='update_question_database.php' method='post'><input type='hidden' name='ques_id' value='" . $row["question_id"] . "'><input type='submit' name='edit' value='  edit  '></form></td><td><form action='' method='post'><input type='hidden' name='q_id' value='" . $row["question_id"] . "'><input type='submit' id='delete_button' name='delete_row' value='delete'></form></td></tr>\n";
                }
              }
            echo"</tbody></table>";
